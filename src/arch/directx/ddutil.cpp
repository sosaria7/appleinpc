...
// both approaches.
HRESULT CDisplay::StretchBlt( RECT *destRect, LPDIRECTDRAWSURFACE7 pSourceSurface, RECT *sourceRect, 
					  DWORD dwFlags, LPDDBLTFX lpDDBltFx )
{
    if( NULL == m_pddsBackBuffer )
        return E_POINTER;
	
    return m_pddsBackBuffer->Blt(destRect, pSourceSurface, sourceRect, dwFlags, lpDDBltFx);
}
...
HRESULT CSurface::DrawBitmapIn(HBITMAP hBMP, DWORD dwX, DWORD dwY, DWORD dwWidth, DWORD dwHeight)
{
    HDC            hDCImage;
    HDC            hDC;
    BITMAP         bmp;
    ...
    // Use the passed size, unless zero
    dwWidth  = ( dwWidth  == 0 ) ? bmp.bmWidth  : dwWidth;     
    dwHeight = ( dwHeight == 0 ) ? bmp.bmHeight : dwHeight;

    // Stretch the bitmap to cover this surface
    if( FAILED( hr = m_pdds->GetDC( &hDC ) ) )
        return hr;

    StretchBlt( hDC, dwX, dwY, 
                dwWidth, dwHeight, 
                hDCImage, 0, 0,
...
HRESULT CSurface::DrawBitmapIn(TCHAR *strBMP, DWORD dwX, DWORD dwY, DWORD dwWidth, DWORD dwHeight)
{
    HBITMAP hBMP;
    HRESULT hr;
    ...
    // Draw the bitmap on this surface
    if( FAILED( hr = DrawBitmapIn( hBMP, dwX, dwY, dwWidth, dwHeight ) ) )
    {
        DeleteObject( hBMP );
        return hr;
    }
